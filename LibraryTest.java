...
public class LibraryTest {
    ...
    public void setUp() throws Exception {
        ...
        abc1 = new Book ("ABCrrr", "Amber");
        abc2 = new Book ("ABCrrr", "Marry");
        abc3 = new Book ("ABCrrr", "Jane");
    ...
    @Test
    public void testCheckOutManyBooks() {
        Patron dave = openAndServeDave();
        library.search("abcr");
    ...
    @Test
    public void testSearchLessThanFourChar() {
        library.open();
        ArrayList<Book> foundBooks = library.search("abc");
        assertEquals(0, foundBooks.size());
    }
    ...
    public void testCheckStringValid () {
    	// valid cases
    	assertTrue(library.checkStringValid("1"));
    	assertTrue(library.checkStringValid("13,75"));
    	assertTrue(library.checkStringValid("1,2,3"));
    	
    	// invalid cases
    	assertFalse(library.checkStringValid("1, 2, 3")); // no space allowed
    	assertFalse(library.checkStringValid("5,"));
    	assertFalse(library.checkStringValid("a")); 
    	assertFalse(library.checkStringValid("abc"));
    	assertFalse(library.checkStringValid("1,2,a")); // a letter at the end wont be tolerant lol
    }
    
    @Test 
    public void testStringToInt() {
    	//stringToInt
    	assertEquals(1 , library.stringToInt("1")[0]);
    	
    	assertEquals(2, library.stringToInt("2,5")[0]);
    	assertEquals(5, library.stringToInt("2,5")[1]);
    	
    	assertEquals(1, library.stringToInt("1,2,3,4,5")[0]);
    	assertEquals(2, library.stringToInt("1,2,3,4,5")[1]);
    	assertEquals(3, library.stringToInt("1,2,3,4,5")[2]);
    	assertEquals(4, library.stringToInt("1,2,3,4,5")[3]);
    	assertEquals(5, library.stringToInt("1,2,3,4,5")[4]);
    	
    	// invalid input will not change the default integer, which remains as 0
    	assertEquals(0, library.stringToInt("1, 2, 3")[0]); 
    	assertEquals(0, library.stringToInt("5,")[0]);
    	assertEquals(0, library.stringToInt("a")[0]);
    	assertEquals(0, library.stringToInt("abc")[0]);
    	assertEquals(0, library.stringToInt("1,2,a")[0]); 
    }
    
    /**
     * to test on patron can have only one card
     */
    @Test
    public void testIsPatronExist() {
    	// issueCard("Dave")
    	openAndGiveBooksToDave();
        Patron dave = library.serve("Dave");
        assertEquals(3, dave.getBooks().size());
        
        // check Dave's existence (case sensitive) 
        assertTrue(library.isPatronExist("Dave"));
        assertFalse(library.isPatronExist("dave"));
        
        // issueCard("Dave") again and it's overwriting the card
        Patron daveDuplicate = openAndServeDave();
        assertEquals(0, daveDuplicate.getBooks().size());   
    }
    
    @Test
    public void testNoOfNonZeroElement () {
    	int[] numArray= new int[20];
    	assertEquals(0, library.noOfNonZeroElement(numArray));
    	numArray[0] = 1;
    	numArray[1] = 3;
    	assertEquals(2, library.noOfNonZeroElement(numArray));
    }
    
    // no test for readLind()
}
